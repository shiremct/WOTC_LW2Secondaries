...
static function X2AbilityTemplate AddArcthrowerStun()
{
	...
	// Disorient on miss (if appropriate effects are active)
	Template.AddTargetEffect(ElectroshockDisorientEffect());

	// Apply effect that will remove stuns on team changes (bug fix)
	RemoveStunOnTeamChange = new class'X2Effect_RemoveStunOnTeamChanges';
	Template.AddTargetEffect(RemoveStunOnTeamChange);
...
// Electroshock - Passive: Attaches a passive icon with effectname to trigger the ElectroshockDisorentEffect() effect
static function X2AbilityTemplate Electroshock()
{
	...
	// Effect to show a passive icon in the tactical UI for the SourceUnit and 
	IconEffect = new class'X2Effect_Persistent';
	IconEffect.BuildPersistentEffect(1, true, false);
...
static function X2Effect_Persistent ElectroshockDisorientEffect()
{
	...
	local X2Condition_SourceEffectRequirement	EffectCondition;
	local X2Condition_UnitProperty				Condition_UnitProperty;
	...
	EffectCondition = new class'X2Condition_SourceEffectRequirement';
	EffectCondition.RequiredEffectNames.AddItem('Arcthrower_DisorientOnMissEffect');
	DisorientedEffect.TargetConditions.AddItem(EffectCondition);
